...
float4 CreateQuaternion(float3 euler)
{
    float cy;
    float sy;
    sincos(euler.z * 0.5, sy, cy);
    float cp;
    float sp;
    sincos(euler.x * 0.5, sp, cp);
    float cr;
    float sr;
    sincos(euler.y * 0.5, sr, cr);
...
float3 MultiplyInverseQuaternion(const float4 quaternionUnity, float3 vector3)
{
    float a = quaternionUnity.w;
    float b = quaternionUnity.x;
    float c = quaternionUnity.y;
    ...
    float t9 = c * d;
    float t10 = -d * d;
    float v1new = 2 * ((t8 + t10) * vector3.x + (t4 + t6) * vector3.y + (t7 - t3) * vector3.z) + vector3.x;
    float v2new = 2 * ((t6 - t4) * vector3.x + (t5 + t10) * vector3.y + (t9 + t2) * vector3.z) + vector3.y;
    float v3new = 2 * ((t3 + t7) * vector3.x + (t9 - t2) * vector3.y + (t5 + t8) * vector3.z) + vector3.z;
